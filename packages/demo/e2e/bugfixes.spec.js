...
describe('bugfixes', function() {
  ...
  if (browser.params.package === 'loader') {
    ...
    it('should update components that are registered in a separate file (issue #4)' // eslint-disable-line max-len
        , async function() {
          const componentFilePath = srcPath('elements', 'button.component.js');
          const originalComponentFile = await readFile(
              componentFilePath,
              'utf8',
          );
          ...
          // .sendKeys() to it, but then we'd run into localization issues if
          // the tests are being run with different browser settings. The value
          // property has the same format regardless of localization.
